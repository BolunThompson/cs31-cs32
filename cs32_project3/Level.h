...
public:
  ...
  Level(std::string assetPath) : m_pathPrefix(assetPath) {
  ...
  LoadResult loadLevel(std::string filename) {
    ...
    for (int y = VIEW_HEIGHT - 1; std::getline(levelFile, line); y--) {
      ...
      if (line.size() < VIEW_WIDTH ||
          line.find_first_not_of(" \t\r", VIEW_WIDTH) != std::string::npos)
        return load_fail_bad_format;
  ...
  bool edgesValid() const {
    for (int y = 0; y < VIEW_HEIGHT; y++)
      if (m_maze[y][0] != floor || m_maze[y][VIEW_WIDTH - 1] != floor)
        return false;
    for (int x = 0; x < VIEW_WIDTH; x++)
      if (m_maze[0][x] != floor || (m_maze[VIEW_HEIGHT - 1][x] != floor &&
                                    m_maze[VIEW_HEIGHT - 1][x] != ladder))
        return false;
