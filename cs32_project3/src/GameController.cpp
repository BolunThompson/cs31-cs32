...
static void convertToGlutCoords(double x, double y, double &gx, double &gy,
                                double &gz);
...
void GameController::initDrawersAndSounds() {
  SpriteInfo drawers[] = {
      {IID_PLAYER, 0, "mario1.tga", "PLAYER", 0},
      {IID_PLAYER, 1, "mario2.tga", "PLAYER", 0},
      {IID_KONG, 0, "kong1.tga", "KONG", 0},
      {IID_KONG, 1, "kong2.tga", "KONG", 0},
      {IID_KONG, 2, "kong3.tga", "KONG", 0},
      {IID_BARREL, 0, "barrel1.tga", "BARREL", 1},
      {IID_BARREL, 1, "barrel2.tga", "BARREL", 1},
      {IID_BARREL, 2, "barrel3.tga", "BARREL", 1},
      {IID_FIREBALL, 0, "fire1.tga", "FIREBALL", 1},
      {IID_KOOPA, 0, "koopa1.tga", "KOOPA", 0},
      {IID_KOOPA, 1, "koopa2.tga", "KOOPA", 0},
      {IID_FLOOR, 0, "wall.tga", "FLOOR", 2},
      {IID_LADDER, 0, "ladder.tga", "LADDER", 3},
      {IID_EXTRA_LIFE_GOODIE, 0, "extralife.tga", "EXTRA_LIFE_GOODIE", 2},
      {IID_GARLIC_GOODIE, 0, "garlic.tga", "GARLIC_GOODIE", 2},
      {IID_BONFIRE, 0, "bonfire1.tga", "BONFIRE", 3},
      {IID_BONFIRE, 1, "bonfire2.tga", "BONFIRE", 3},
      {IID_BURP, 0, "gascloud.tga", "BURP", 1},
  };

  m_soundMap = {
      {SOUND_THEME, "theme.wav"},       {SOUND_ENEMY_DIE, "enemydie.wav"},
      {SOUND_PLAYER_DIE, "death.wav"},  {SOUND_BURP, "burp.wav"},
      {SOUND_GOT_GOODIE, "goodie.wav"}, {SOUND_FINISHED_LEVEL, "finished.wav"},
      {SOUND_JUMP, "jumpbar.wav"},
  };
  ...
  for (const auto &d : drawers) {
    ...
    if (!path.empty())
      path += '/';
    if (!m_spriteManager.loadSprite(path + d.tgaFileName, d.imageID,
                                    d.frameNum)) {
...
static void doSomethingCallback() { Game().doSomething(); }
...
void GameController::run(int argc, char *argv[], GameWorld *gw,
                         string windowTitle, int msPerTick) {
...
void GameController::keyboardEvent(unsigned char key, int /* x */,
                                   int /* y */) {
  switch (key) {
  case 'a':
  case '4':
...
void GameController::quitGame() { setGameState(quit); }
...
void GameController::displayGamePlay() {
  glEnable(GL_DEPTH_TEST); // must be done each time before displaying graphics
                           // or gets disabled for some reason
  glLoadIdentity();
  ...
  for (int i = GraphObject::NUM_DEPTHS - 1; i >= 0; --i) {
    for (auto it = graphObjects.begin(); it != graphObjects.end(); it++) {
      ...
      if (m_imageDepthMap.at(cur->getID()) == i && cur->isVisible()) {
        ...
        m_spriteManager.plotSprite(imageID,
                                   cur->getAnimationNumber() %
                                       m_spriteManager.getNumFrames(imageID),
                                   gx, gy, gz, angle, cur->getSize());
      }
    }
  }
...
void GameController::reshape(int w, int h) {
  ...
  glLoadIdentity();
#ifdef _MSC_VER
  gluPerspective(45.0, double(WINDOW_WIDTH) / WINDOW_HEIGHT,
...
static void doOutputStroke(double x, double y, double z, double size,
                           const char *str, bool centered) {
  if (centered) {
    double len =
  ...
  glLineWidth(1);
  glLoadIdentity();
  glTranslatef(static_cast<GLfloat>(x), static_cast<GLfloat>(y),
...
// static void outputStroke(double x, double y, double z, double size, const
// char* str)
//{
...
static void drawScoreAndLives(string gameStatText) {
  static int RATE = 1;
  static GLfloat rgb[3] = {static_cast<GLfloat>(.6), static_cast<GLfloat>(.6),
                           static_cast<GLfloat>(.6)};
  for (int k = 0; k < 3; k++) {
