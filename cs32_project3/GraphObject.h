...
public:
  ...
  GraphObject(int imageID, int startX, int startY, int dir = 0,
              double size = 1.0)
  ...
  virtual ~GraphObject() { getGraphObjects().erase(this); }
  ...
  void getPositionInThisDirection(int dir, int units, int &newX,
                                  int &newY) const {
    newX = getX();
    newY = getY();
    switch (dir) {
