...
public:
  ...
  bool loadSprite(std::string filename_tga, int imageID, int frameNum) {
    ...
    m_frameCountPerSprite[imageID]++; // keep track of how many frames per
                                      // sprite we loaded
    ...
    unsigned char byteCount =
        static_cast<unsigned char>(header.pixel_depth) / 8;
    const long imageSize =
        header.width_pixels * header.height_pixels * byteCount;
    ...
    if (!tgaFile) {
      std::cerr << "***** Unable to read " << imageSize
                << " (imageSize) bytes from file " << filename_tga << std::endl;
      return false;
    }
    ...
    if (header.color_map_type != 0 ||
      ...
      std::cerr << "***** Bad color_map_type or image type in " << filename_tga
                << std::endl;
      return false;
    }
    ...
    if (header.image_descriptor & 0x20) // image ios flipped vertically
      flipVertical(imageData.get(), header.width_pixels, header.height_pixels,
                   byteCount);
    ...
    if (m_mipMapped) {
      ...
      if (3 == byteCount)
        glTexImage2D(GL_TEXTURE_2D, 0, 3, textureWidth, textureHeight, 0,
                     GL_BGR, GL_UNSIGNED_BYTE, imageData.get());
      else if (4 == byteCount)
        glTexImage2D(GL_TEXTURE_2D, 0, 4, textureWidth, textureHeight, 0,
                     GL_BGRA, GL_UNSIGNED_BYTE, imageData.get());
    }
  ...
  bool plotSprite(int imageID, int frame, double gx, double gy, double gz,
    ...
    // #define FULL_ROTATION	// for games where you can rotate 360 degrees,
    // not just n/s/e/w
    ...
    static_cast<GLfloat>(finalHeight), 0); glTexCoord2d(cx4, cy4); glVertex3f(0,
    static_cast<GLfloat>(finalHeight), 0); glEnd();
    */
...
#pragma pack(1)
  struct TGA_HEADER {
    ...
    unsigned char image_descriptor; // bits 3-0 give alpha channel depth, and
                                    // 5-4 give direction.
  };
#pragma pack()
  ...
  static void makeMipmaps(unsigned char byteCount, unsigned int textureWidth,
                          unsigned int textureHeight, char *imageData) {
...
#else
    gluBuild2DMipmaps(GL_TEXTURE_2D, byteCount, textureWidth, textureHeight,
